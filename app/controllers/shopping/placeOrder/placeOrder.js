...
const PlaceOrder = require('../../../models/shopping/placeOrderModel');
const productModel = require('../../../models/shopping/productModel');
...
Routes 
/placeOrder get
/placeOrder post add placeOrder
/buyplaceOrder
*/
//to retrive all placeOrder
const placeOrder = {};

placeOrder.getAllPlaceOrder = async (callback, event) => {
    try {
...
//to add placeOrder
placeOrder.placeOrder = async (callback, event) => {
    ...
    try {
        ...
        const newSubs = await PlaceOrder.placeOrder(requestBody);
        if (!newSubs) {
...
//to Update placeOrder
placeOrder.updateOrderStatus = async (callback, event) => {
    const requestBody = JSON.parse(event.body);
    if (!requestBody) sendResponse(callback, 400, 'PlaceOrder object not found', null)
    try {
        if (!requestBody.client_id) requestBody.client_id = event.headers.client_id;
...
//to delete placeOrder
placeOrder.deleteOrder = async (callback, event) => {
...
placeOrder.orderPaymentIntent = async (callback, event) => {
    const requestBody = JSON.parse(event.body);
